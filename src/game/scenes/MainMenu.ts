...
export default class MainMenu extends Phaser.Scene {
    ...
    showTotal(userInfo: any, size: any) {
        if (!this.totalScore) {
        ...
        this.totalScoreText = this.add.text(size.width / 2, size.height / 2 - 200, `Score:${this.totalScore || 0}`, this.fontStyle).setOrigin(0.5, 0.5);
        ...
        const highScoreText = this.add.text(size.width / 2, size.height / 2 - 150, 'High Score: ' + `${this.maxScore || 0}`, { ...this.fontStyle, fontSize: 24 }).setOrigin(0.5, 0.5).setAlpha(0);
        this.tweens.add({
