...
import { IRefPhaserGame, PhaserGame } from '@/components/MemoryGame/PhaserGame';
...
function GamePage() {
  ...
  const [result, setResult] = useState<any>('fail')
  ...
  const currentActiveScene = async (scene: Phaser.Scene) => {
    ...
    if (scene.scene.key == 'GameOver') {
      ...
      if (parseInt(_score)) {
        ...
        endGameReq({ score: _score * 1, result: _result }).then(res => {
          if (res.code == 0) {
    ...
    <div className='game-wrapper'>
      {
        currentScene == 'GameOver' ? <div className='game-over'>
          <div className='game-over-top'>
            ...
            {
              result == 'win' ? <div className='score-wrapper'>
                <span>+</span>
                <span>{1} </span>
                <img src="/assets/common/ticket.png" alt="tomato" />
              </div> : null
            }
          </div>
          <div className='game-over-bots'>
            ...
            <div className='game-over-btn play-btn' onClick={() => restartGame()}>
              <div className='game-over-bot-middle'>
                {
                  result == 'win' ? `Continue` : `Play (${userInfo.ticket} Attempts Left)`
                }
              </div>
            </div>
          </div>
